...
class UpdateManager:
    ...
    def check_updates() -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if sys.platform == "darwin":
                # macOS: 优先查找 .dmg，其次 .zip
                suffixes = (".dmg", ".zip")
            else:
                suffixes = (".exe",)
            for asset in latest_release.get("assets", []):
                asset_name = asset.get("name", "")
                if any(asset_name.endswith(s) for s in suffixes):
                    download_url = asset.get("browser_download_url")
                    file_name = asset.get("name")
                    break

            if not download_url:
                logging.warning(f"Release 中没有找到适用于当前平台的安装包 (期望后缀: {suffixes})")
                return {"has_update": False, "status": "unknown"}
    ...
    def cleanup_old_executables(exclude_path: str):
        """删除目录下旧版本的安装包文件（保留 exclude_path 本体）。"""
        if not exclude_path:
            return
        ...
        # 按平台决定要清理的文件后缀
        if sys.platform == "darwin":
            cleanup_suffixes = (".dmg", ".zip")
        else:
            cleanup_suffixes = (".exe",)

        try:
            exclude_norm = os.path.normcase(os.path.abspath(exclude_path))
            for file in os.listdir(directory):
                if not any(file.lower().endswith(s) for s in cleanup_suffixes):
                    continue
    ...
    def schedule_running_executable_deletion(exclude_path: str):
        """调度在当前进程退出后删除正在运行的可执行文件（仅 Windows）"""
        if sys.platform != "win32":
            # macOS .app bundle 通过 DMG 覆盖安装，无需此机制
            return
